  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12 max-w-7xl mx-auto px-2">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500 font-mono">{t.operatorLabel} APMC-NSK-402</span>
              </div>
              <h1 className="text-2xl font-black text-slate-900 mt-1">
                {t.staffTerminalHeader}
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-2xl border border-slate-200">
            <div className="text-right px-2">
              <div className="text-[10px] text-slate-500 font-black uppercase">{t.currentlyServingLabel}</div>
              <div className="text-lg font-black text-amber-600 font-mono">
                {activeBooking?.currentServingToken || "SOY-2026-MH33"}
              </div>
            </div>
            <button
      ...
      <div className={`grid grid-cols-1 ${isMobilePreview ? 'grid-cols-1' : 'lg:grid-cols-12'} gap-6 sm:gap-8`}>
        ...
        {/* Left Column: Quality Inspection & Weighbridge Form */}
        <div className={`${isMobilePreview ? 'col-span-1' : 'lg:col-span-7'} space-y-6`}>
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200 space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-black text-slate-900">
                    {t.qualityWeighbridgeTitle}
                  </h2>
                  <p className="text-xs text-slate-500 font-medium">{t.qualityWeighbridgeSub}</p>
                </div>
              </div>
              <span className="font-mono font-black text-slate-700 bg-slate-100 px-3 py-1 rounded-xl text-xs">
                {t.tokenLabel} {activeBooking?.tokenId}
              </span>
            </div>
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-emerald-100/60 border border-emerald-200 p-4 rounded-2xl flex flex-wrap justify-between items-center text-xs">
              <div>
                <span className="text-slate-500 font-bold">{t.farmerLabel} </span>
                <span className="font-black text-slate-900">{activeBooking?.farmerName}</span>
              </div>
              <div>
                <span className="text-slate-500 font-bold">{t.cropLabel} </span>
                <span className="font-black text-emerald-800">{activeBooking?.cropLabel}</span>
              </div>
              <div>
                <span className="text-slate-500 font-bold">{t.declaredQtyLabel} </span>
            ...
            <form onSubmit={handleSaveQualityWeighment} className="space-y-6 text-xs">
              ...
              <div className="space-y-4">
                <div className="font-black text-slate-900 uppercase tracking-wider text-xs flex items-center gap-2 text-emerald-700">
                  ...
                  <span>{t.qualityInspectionHeading}</span>
                </div>

                <div className={`grid grid-cols-1 ${isMobilePreview ? 'grid-cols-1' : 'sm:grid-cols-3'} gap-4`}>
                  <div>
                    <label className="block font-black text-slate-700 mb-1.5">
                      {t.moistureInputLabel}
                    </label>
                    <input
                      type="number"
                      step="0.1"
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Standard">{t.standardGradeOption}</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100">
                <div className="font-black text-slate-900 uppercase tracking-wider text-xs flex items-center gap-2 text-emerald-700">
                  ...
                  <span>{t.weighbridgeHeading}</span>
                </div>
                ...
                <div className="bg-amber-50 border-2 border-amber-300 p-4 rounded-2xl flex items-center justify-between">
                  <div>
                    <div className="text-[10px] text-amber-900 font-black uppercase">{t.verifiedNetGrainWeight}</div>
                    <div className="text-base font-black text-amber-950 font-mono">
                      {netKg.toLocaleString()} kg ({netQuintals} Quintals)
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] text-amber-900 font-black uppercase">{t.calculatedMspBill}</div>
              ...
              >
                ...
                <span>{t.saveQualityWeighmentBtn}</span>
              </button>
            </form>
          </div>
        </div>
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="bg-slate-50 border-2 border-slate-300 rounded-2xl p-5 space-y-3 font-mono text-xs shadow-inner">
                  <div className="text-center pb-2 border-b border-slate-200">
                    <div className="font-black text-slate-900 text-base">{t.fciProcurementReceipt}</div>
                    <div className="text-[10px] text-slate-500 font-sans">Nashik APMC Mandi • Receipt #APMC-2026-9910</div>
                  </div>

                  <div className="space-y-2 text-xs font-sans">
                    <div className="flex justify-between">
                      <span className="text-slate-500">{t.farmerNameLabel}:</span>
                      <span className="font-black text-slate-900">{activeBooking?.farmerName}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-500">{t.verifiedNetQty}</span>
                      <span className="font-black text-slate-900 font-mono">{netQuintals} Quintals</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">{t.govtMspRateLabel}</span>
                    ...
                    <div className="flex justify-between pt-2 border-t border-slate-200 font-black">
                      <span className="text-slate-800">{t.totalMspPayable}</span>
                      <span className="text-emerald-800 text-base font-mono">₹{totalMspAmount.toLocaleString('en-IN')}</span>
                    </div>
                  </div>
                ...
                <div className="bg-emerald-50 border-2 border-emerald-300 p-5 rounded-2xl space-y-3">
                  <div className="text-xs text-emerald-950 font-bold">
                    {t.dbtReadyDesc} <span className="font-black">Bank Account ****4821</span>.
                  </div>
                  ...
                  {activeBooking?.stepIndex >= 7 ? (
                    <div className="p-4 bg-white border-2 border-emerald-300 rounded-xl text-center space-y-1 shadow-md">
                      ...
                      <div className="text-sm font-black text-emerald-900">{t.dbtExecutedTitle}</div>
                    ...
                    >
                      ...
                      <span>{t.initiateDbtBtn} (₹{totalMspAmount.toLocaleString('en-IN')})</span>
                    </button>
                  )}
                </div>
              </div>
